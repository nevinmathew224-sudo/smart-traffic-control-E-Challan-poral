...
$filterActive = $search !== '' || $statusFilter !== 'all';
...
if ($userTableExists) {
    $query = "
        SELECT
            u.id,
            u.email,
            COALESCE(u.vehicle_no, MAX(c.vehicle_no)) AS vehicle_no,
            u.role,
    ...
    if ($search !== '') {
        $query .= " AND (u.email LIKE ? OR CAST(u.id AS CHAR) LIKE ? OR u.vehicle_no LIKE ? OR c.vehicle_no LIKE ?)";
        $like = '%' . $search . '%';
        $params[] = $like;
        $params[] = $like;
        $params[] = $like;
    ...
    $query .= " GROUP BY u.id, u.email, u.vehicle_no, u.role";
...
$storedUserCount = 0;
if ($userTableExists) {
    $storedUserCount = (int) (mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM users WHERE role = 'user'"))['c'] ?? 0);
}
if (count($users) === 0 && !$filterActive && $storedUserCount === 0) {
    $usingDemoData = true;
    $users = [
        ['id' => 11, 'email' => 'aarav.sharma@trafficdemo.com', 'vehicle_no' => 'DL01AB1234', 'role' => 'user', 'total_cases' => 3, 'paid_cases' => 1, 'unpaid_cases' => 2, 'total_fine' => 4500, 'pending_amount' => 3000, 'latest_challan' => 250],
        ['id' => 21, 'email' => 'ananya.sharma@trafficdemo.com', 'vehicle_no' => 'DL02CD5678', 'role' => 'user', 'total_cases' => 2, 'paid_cases' => 1, 'unpaid_cases' => 1, 'total_fine' => 2500, 'pending_amount' => 1000, 'latest_challan' => 232],
        ['id' => 31, 'email' => 'rohan.malhotra@trafficdemo.com', 'vehicle_no' => 'DL03EF9012', 'role' => 'user', 'total_cases' => 2, 'paid_cases' => 2, 'unpaid_cases' => 0, 'total_fine' => 3000, 'pending_amount' => 0, 'latest_challan' => 221],
        ['id' => 42, 'email' => 'riya.bajaj@trafficdemo.com', 'vehicle_no' => 'DL04GH3456', 'role' => 'user', 'total_cases' => 1, 'paid_cases' => 0, 'unpaid_cases' => 1, 'total_fine' => 1000, 'pending_amount' => 1000, 'latest_challan' => 232],
        ['id' => 53, 'email' => 'sameer.khan@trafficdemo.com', 'vehicle_no' => 'DL05JK7890', 'role' => 'user', 'total_cases' => 1, 'paid_cases' => 1, 'unpaid_cases' => 0, 'total_fine' => 1000, 'pending_amount' => 0, 'latest_challan' => 243],
    ];
}
    ...
    <main class="main">
        <div class="topbar">
            ...
            <div class="profile-card">
                ...
                <div class="toolbar">
                    <a href="challans.php" class="primary">Open Challans</a>
        ...
        <div class="notice">
            ...
            $vnoParam = trim($_GET['vno'] ?? '');
            if ($message === 'created' && $uidParam > 0) {
                echo 'Citizen account active! Unique User ID: <strong>#' . e((string) $uidParam) . '</strong>' . ($vnoParam !== '' ? ' • Vehicle: <strong>' . e($vnoParam) . '</strong>' : '') . ' • User can log in with User ID or Vehicle Number and their set password.';
            } else {
                $messages = [
        ...
        <section class="panel">
            ...
            <form class="create-form" method="POST" action="save_user.php">
                ...
                <div>
                    ...
                    <input id="new_vehicle_no" type="text" name="vehicle_no" placeholder="e.g. KL07AB1234" style="text-transform:uppercase;">
                </div>
                <div>
        ...
        <section class="panel">
            ...
            <p>Filter citizens by email, vehicle number, user ID, or payment status.</p>
            <form class="filter-form" method="GET">
                <div>
                    <label for="search">Search user</label>
                    <input id="search" name="search" value="<?php echo e($search); ?>" placeholder="Search by email, vehicle no, or user ID">
                </div>
                <div>
        ...
        <section class="panel">
            ...
            <div class="table-wrap">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($user['vehicle_no'])) { ?>
                            <span class="plate-tag"><?php echo e($user['vehicle_no']); ?></span>
                            <?php } else { ?>
            ...
            <div class="note">No users match the current search or filter selection.</div>
            <?php } else { ?>
