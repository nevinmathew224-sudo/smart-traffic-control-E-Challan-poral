...
if ($q !== '') {
    ...
    $stmt = $conn->prepare("SELECT id, email, vehicle_no, role FROM users WHERE email LIKE ? OR CAST(id AS CHAR) LIKE ? OR vehicle_no LIKE ? ORDER BY id ASC LIMIT 8");
    $stmt->bind_param("sss", $like, $like, $like);
    $stmt->execute();
    $result = $stmt->get_result();
    while ($row = $result->fetch_assoc()) {
...
<main class="wrap">
    ...
    <section class="panel">
        <h3>User Results</h3>
        <table>
            <tr><th>ID</th><th>Email</th><th>Vehicle</th><th>Role</th><th>Open</th></tr>
            <?php foreach ($users as $row) { ?>
            <tr><td>#<?php echo e((string) $row['id']); ?></td><td><?php echo e($row['email']); ?></td><td><?php echo e($row['vehicle_no'] ?: '—'); ?></td><td><?php echo e($row['role']); ?></td><td><a href="users.php?search=<?php echo urlencode($row['email']); ?>">Open</a></td></tr>
            <?php } ?>
        </table>
    </section>
