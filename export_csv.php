...
if ($type === 'users') {
    fputcsv($out, ['User ID', 'Email', 'Vehicle No', 'Role', 'Total Cases', 'Paid Cases', 'Unpaid Cases', 'Pending Amount']);
    $result = mysqli_query($conn, "
        SELECT u.id, u.email, COALESCE(u.vehicle_no, MAX(c.vehicle_no), 'N/A') AS vehicle_no, u.role,
        ...
        WHERE u.role = 'user'
        GROUP BY u.id, u.email, u.vehicle_no, u.role
        ORDER BY u.id ASC
    ");
